...
__device__ void bomba2(int* dev_tablero,int explota, int filas, int columnas) {

	int i = blockIdx.y * blockDim.y + threadIdx.y;		//Indice de la x
	int j = blockIdx.x * blockDim.x + threadIdx.x;		//Indice de la y

														//Identificamos los diamantes por su columna
	if (j == explota&& i<filas)	dev_tablero[i*columnas + j] = 0;
...
__device__ void memoriaCompartida(int* dev_tablero, int fila1, int columna1, int tamFilas, int tamColumnas,int valor, int* dev_contadorEliminados) {
	...
	__shared__ int matrizCompartida[TESELA][TESELA];			//Variable compartida

	int x = blockIdx.y * blockDim.y + threadIdx.y;				//Indice de las x
	int y = blockIdx.x * blockDim.x + threadIdx.x;				//Indice de las y
	...
	//Se rellena la memoria compartida con los valores correspondientes
	matrizCompartida[threadIdx.y][threadIdx.x] = dev_tablero[(x*tamColumnas) + y];
	...
	__syncthreads();
			...
			if (sameHorizon1 > sameVertical1) {
				...
				if ( y == columna1 && dev_tablero[(x * tamColumnas) + y] == valor) {			
					matrizCompartida[threadIdx.y][threadIdx.x] = 0;	
				}
			}
			if (x == fila1 && y == columna1) {				//Centro
				matrizCompartida[threadIdx.y][threadIdx.x] = 0;
			}
			...
			//Tras ser los valores modificados, se pasan a la memoria global
			dev_tablero[(x * tamColumnas) + y] = matrizCompartida[threadIdx.y][threadIdx.x];
}
...
__global__ void jugarKernel(int* dev_tablero, int fila1, int columna1, int fila2, int columna2, int tamFilas, int tamColumnas, int* dev_contadorEliminados) {
	...
	if (comprobarMayor(sameVertical1, sameHorizon1, sameVertical2, sameHorizon2)) { //comprobamos cual de las posiciones cambiadas explota mas
		...
		int valor = dev_tablero[(fila1 * tamColumnas) + columna1]; // Valor que tendra la celda eliminada
		memoriaCompartida(dev_tablero, fila1, columna1, tamFilas, tamColumnas, valor, dev_contadorEliminados);
	}else {
		int valor = dev_tablero[(fila2 * tamColumnas) + columna2]; // Valor que tendra la celda eliminada
		memoriaCompartida(dev_tablero, fila2, columna2, tamFilas, tamColumnas, valor, dev_contadorEliminados);
	}

	__syncthreads();

	reestructuracionArribaAbajo(dev_tablero, tamFilas, tamColumnas);
...
cudaError_t jugar(int* tablero, int tamFilas, int tamColumnas, int* contadorEliminados, char m, int nColores) {
	...
	bloquex = tamFilas / TESELA;
	bloquey = tamColumnas / TESELA;

	if (tamFilas % TESELA != 0) {
		bloquex++;
	}

	if (tamColumnas % TESELA != 0) {
		bloquey++;
	}

	dim3 blocks(bloquex, bloquey);
	dim3 threads(TESELA, TESELA);
